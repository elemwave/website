...
Parameter Store owns those credentials. The workflow reads them and exports
them before running `cdk`; locally, do the same:
...
1. **Bootstrap both regions.** The workflow does this on every run, so the only
   reason to run it by hand is to deploy the certificate before the first
   workflow run. The certificate lives in `us-east-1`, everything else in
   `eu-west-1`.
...
2. **Check the deployment role.** It is maintained by hand in the account as
   ...
   it can deploy the site stack, to read the staging credentials from Parameter
   Store, to write the site bucket, and to invalidate the distribution. Distribution ids are generated by CloudFront, so the
   ...
   certificate stack is deployed by an operator, and the cross-region reference
   is read by a resource inside the `eu-west-1` stack, under its own role.
