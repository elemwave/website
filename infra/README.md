...
2. **Check the deployment role.** It is maintained by hand in the account as
   ...
   It needs nothing in `us-east-1`: the certificate stack is deployed by an
   operator, and the cross-region reference is read by a resource inside the
   `eu-west-1` stack, under its own role.
