      ...
      <div className="mx-auto mt-[clamp(36px,5vw,56px)] max-w-[1180px]">
        ...
        <div className="grid">
          {SLIDES.map((s, i) => (
            <div
              key={s.caption}
              aria-hidden={i !== slide}
              className={`col-start-1 row-start-1 flex flex-wrap content-center items-center justify-center gap-[var(--logo-gap)] [--logo-gap:clamp(24px,4vw,48px)] ${i === slide ? "" : "invisible"}`}
              style={{ "--per": perRow(s.logos.length) } as CSSProperties}
            >
              {s.logos.map((src) => (
                <div
                  key={src}
                  className={`flex w-[calc((100%-var(--logo-gap)*(var(--per)-1))/var(--per))] justify-center ${FULL_ROW_AT[s.logos.length] ?? ""}`}
                >
        ...
        <div className="relative mt-11">
          ...
          <button
            ...
            className={`${arrowClass} left-[max(-16px,-2vw)]`}
          >
            ‹
          </button>
          <button
            ...
            className={`${arrowClass} right-[max(-16px,-2vw)]`}
          >
            ›
          </button>
        </div>
        ...
        <div className="mt-7 flex justify-center gap-[10px]">
          {SLIDES.map((s, i) => (
            <button
              ...
              className={`h-[10px] w-[10px] cursor-pointer rounded-full border-none p-0 ${i === slide ? "bg-navy-950" : "bg-dot-idle"}`}
            />
          ))}
        </div>
      </div>
